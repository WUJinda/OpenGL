...
{
    /*
    Détectez l'état d'appuyer sur les touches fléchées gauche et droite
    pour contrôler le mouvement de la voiture du joueur à chaque fois qu'elle est rafraîchie.
    */
    ...
    //Lorsque le carburant est épuisé, le jeu se termine.
    if(!(maVoiture_->getOil() > 0)){
        warning();
    }
    //Lorsque les voitures qui roulent les unes sur les autres se heurtent, le jeu est terminé.
    detectCollision();
    //Lorsque la voiture opposée sort de la limite, elle est supprimée du vecteur.
    oppositeCars_->moveOut();
    //Lorsque la voiture opposée sort de la limite, une nouvelle voiture sera générée et ajoutée au vecteur.
    ...
    /*
     * Dans l'état sans pause, le temps continue d'augmenter,
     * le carburant continue d'être consommé et le score augmente en conséquence.
    */
    if(!pause_){
        increaseTime();
        record_ += 1;
